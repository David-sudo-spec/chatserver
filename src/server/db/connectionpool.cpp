...
ConnectionPool::ConnectionPool()
{
    ...
    // 启动一个新的定时线程，扫描超过maxIdleTime时间的空闲连接，进行队列的连接回收
...
bool ConnectionPool::loadConfigFile()
{
    ...
    if (pf == nullptr)
    {
        LOG_INFO << __FILE__ << ":" << __LINE__ << ":" << "mysql.conf file is not exist!";
        return false;
    }
...
std::shared_ptr<Connection> ConnectionPool::getConnection()
{
    ...
    while (_connectionQue.empty())
    {
        if (std::cv_status::timeout == cv.wait_for(lock, std::chrono::milliseconds(_connectionTimeout)))
        {
            ...
            {
                LOG_INFO << __FILE__ << ":" << __LINE__ << ":" << "获取空闲连接超时了...获取连接失败！";
                return nullptr;
            }
        }
    }
    ...
    /*
     * shared_ptr智能指针析构时，会把connection资源直接delete掉，相当于调用connection的析构函数，close掉了
